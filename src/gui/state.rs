...
pub struct AppState {
    ...
    pub warp_edit: Option<(u64, opengel::core::warp::Warp)>,
...
impl AppState {
    ...
    /// Fit a NURBS [`Warp`](opengel::core::warp::Warp) to the current analysis:
    ...
    pub fn fit_warp(&self) -> Option<opengel::core::warp::Warp> {
    ...
    /// The warp fit purely from the analysis bands (ignoring any manual edit).
    fn auto_fit_warp(&self) -> Option<opengel::core::warp::Warp> {
        use opengel::core::warp::{Anchor, Warp};
        let a = self.analysis()?;
        ...
        // Order lanes left→right; u is the normalized lane index.
        let mut lanes: Vec<&Lane> = a.lanes.iter().collect();
        lanes.sort_by(|l, m| (l.x_min + l.x_max).cmp(&(m.x_min + m.x_max)));
        let nlanes = lanes.len();
        let order: std::collections::HashMap<u32, usize> =
            lanes.iter().enumerate().map(|(i, l)| (l.id, i)).collect();
        let denom = nlanes.saturating_sub(1).max(1) as f64;
        let mut anchors = Vec::new();
        for b in &a.bands {
            let Some(&idx) = order.get(&b.lane_id) else {
                continue;
            };
            let lane = lanes[idx];
            let u = if nlanes <= 1 { 0.5 } else { idx as f64 / denom };
            let v = b.rf.unwrap_or(b.y_center / h.max(1.0)).clamp(0.0, 1.0);
            let cx = (lane.x_min + lane.x_max) as f64 / 2.0;
            anchors.push(Anchor {
                u,
                v,
                x: cx,
        ...
        let nu = nlanes.clamp(2, 5);
        let nv = 5;
        Some(Warp::fit(&anchors, w, h, nu, nv, 0.5))
    }
    ...
    pub fn hover_size_label(&self, ny: f32) -> Option<String> {
        ...
        let h = self.view_image()?.height() as f64;
        let size = fit.size_at(ny as f64 * h);
    ...
    pub fn press_annotation(&mut self, nx: f64, ny: f64) -> bool {
        ...
        let lane_at = |x: f64| -> Option<u32> {
            a.lanes
                .iter()
                .find(|l| x >= l.x_min as f64 && x <= l.x_max as f64)
                .map(|l| l.id)
        };
        ...
        for b in &a.bands {
            ...
            if px < lane.x_min as f64 || px > lane.x_max as f64 {
                continue;
            }
            let dy = (py - b.y_center).abs();
            let tol = (b.y_half_width * 2.0).max(8.0);
        ...
        match sel {
            Some(Selection::Lane(id)) => {
                if let Some(lane) = a.lanes.iter_mut().find(|l| l.id == id) {
                    let half = (lane.x_max - lane.x_min) as f64 / 2.0;
                    let x0 = (px - half).clamp(0.0, w - 2.0 * half).max(0.0);
                    lane.x_min = x0 as u32;
                    lane.x_max = (x0 + 2.0 * half).min(w) as u32;
                }
            }
            Some(Selection::Band(id)) => {
                ...
                let target_lane = a
                    .lanes
                    .iter()
                    .find(|l| px >= l.x_min as f64 && px <= l.x_max as f64)
                    .map(|l| (l.id, l.y_min as f64, l.y_max as f64));
                if let Some(b) = a.bands.iter_mut().find(|b| b.id == id) {
                    b.y_center = py.clamp(0.0, h);
                    if let Some((lid, y0, y1)) = target_lane {
                        b.lane_id = lid;
                        b.rf = Some(((b.y_center - y0) / (y1 - y0).max(1.0)).clamp(0.0, 1.0));
                    }
                }
            }
            None => {}
