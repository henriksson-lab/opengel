...
fn parse_channel(part: &Part<'_>, index: usize) -> Result<(ScnChannel, bool)> {
    ...
    let zero_is_white =
        element_attr(root, "image", "zero_is").is_some_and(|v| v.eq_ignore_ascii_case("white"));
...
impl<'a> Part<'a> {
    ...
    fn split_document(bytes: &'a [u8]) -> Result<Vec<Part<'a>>> {
        let (headers, body) =
            split_headers(bytes).ok_or(ScnError::NotAScan("no MIME header block at the start"))?;
        let headers = String::from_utf8_lossy(headers);
        if header_value(&headers, "Content-Type").is_none() {
            return Err(ScnError::NotAScan("no Content-Type header"));
        }
        let boundary =
            boundary_of(&headers).ok_or(ScnError::NotAScan("Content-Type declares no boundary"))?;
...
mod tests {
    ...
    fn synthetic(channels: usize, max_value: u32, zero_is: &str) -> Vec<u8> {
        ...
        let mut header_xml = String::from(
            "<!DOCTYPE XML>\n<root version=\"1\">\n <name>synthetic</name>\n <user>tester</user>\n",
        );
        ...
        for i in 0..channels {
            ...
            out.extend_from_slice(
                format!("Content-Description: ScanImageTag{i}\r\n\r\n").as_bytes(),
            );
