...
use crate::model::{Analysis, GelImage, GelProject, GelType, FORMAT_VERSION};
...
impl GelDocument {
    ...
    pub fn from_frames(
        ...
        metas: Vec<crate::model::CaptureMeta>,
    ) -> Self {
    ...
    pub fn working_image(&self) -> Option<crate::GrayF32> {
        ...
        if let Some((_, idxs)) = bracket {
            let frames: Vec<crate::GrayF32> = idxs
                .iter()
                .map(|&i| crate::GrayF32::from_dynamic(&self.frames[i]))
                .collect();
            ...
            if exposures.iter().all(|&t| t > 0.0) {
                if let Ok(merged) = crate::hdr::merge_hdr(&frames, &exposures) {
                    return Some(merged);
                }
            }
        }
        Some(crate::GrayF32::from_dynamic(&self.frames[0]))
    }
