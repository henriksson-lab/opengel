...
use gel_core::model::GelType;
use gel_core::{ladders, GelDocument};
...
fn cmd_analyze(
    ...
    let analysis = gel_detect::analyze(&img, gt, &params, &[], min_r2);
    println!(
...
fn cmd_simulate(
    ...
    let gels = gel_sim::simulate_random_batch(count, seed, upright);
    let n = gel_sim::write_dataset(dir, &gels)?;
    println!(
...
fn cmd_eval(dir: &std::path::Path) -> Result<()> {
    use gel_detect::detector::GelDetector;
    use gel_detect::eval::{aggregate, evaluate, GroundTruth};
    use gel_detect::ClassicalDetector;
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        let img = gel_core::GrayF32::from_dynamic(&dynimg);
